  ...
  if (error) {
    ...
    if (catalogueData && catalogueData.status === 200) {
      if (!isBuilt) {
        ...
        isBuilt = true
        body = <DataTable headers={[...headers, ...extraHeaders]} markAll={true} markAllCallback={handleCheckboxChangeAll} markAllChecked={allMarkRowChecked}>{rows.map((row, index) => ( <BrowseRow key={index} index={index} extraHeaders={extraHeaders} row={row} search={search} topicFiltered={topicFiltered} tagFiltered={tagFiltered} extraHeaderTypes={extraHeaderTypes} markRowCallback={handleSelectCheckboxChange} markRowChecked={markRowChecked}></BrowseRow>))}</DataTable>
      }
